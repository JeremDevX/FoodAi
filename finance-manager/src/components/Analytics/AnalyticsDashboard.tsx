...
export default function AnalyticsDashboard() {
  ...
  // Monthly analysis
  const monthlyData = (() => {
    const dataMap = new Map<string, { income: number; expenses: number; balance: number }>();
    
    filteredTransactions.forEach(transaction => {
      const monthKey = new Date(transaction.date).toISOString().slice(0, 7); // YYYY-MM
      const existing = dataMap.get(monthKey) || { income: 0, expenses: 0, balance: 0 };
  ...
  const categoryData = (() => {
    ...
    filteredTransactions
      ...
      .forEach(transaction => {
        const existing = dataMap.get(transaction.category) || { amount: 0, count: 0 };
  ...
  const totalCategoryExpenses = categoryData.reduce((sum, item) => sum + item.amount, 0);
  categoryData.forEach(item => {
    item.percentage = totalCategoryExpenses > 0 ? (item.amount / totalCategoryExpenses) * 100 : 0;
  });
  ...
  const trendsData = (() => {
    ...
    return monthlyData.map((item, index) => {
      ...
      const incomeTrend = prevItem.income > 0 ? ((item.income - prevItem.income) / prevItem.income) * 100 : 0;
      const expenseTrend = prevItem.expenses > 0 ? ((item.expenses - prevItem.expenses) / prevItem.expenses) * 100 : 0;
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Période: {formatShortDate(selectedDateRange.start)} - {formatShortDate(selectedDateRange.end)}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-success/10 p-4 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm text-success mb-1">Revenus Totaux</div>
          ...
          <div className="bg-danger/10 p-4 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm text-danger mb-1">Dépenses Totales</div>
          ...
          <div className="bg-financial-100 p-4 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={`text-2xl font-bold ${totalIncome - totalExpenses >= 0 ? 'text-success' : 'text-danger'}`}>
                  {formatCurrency(totalIncome - totalExpenses)}
                </div>
              </div>
              <DollarSign className="h-8 w-8 text-financial-600" />
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        {viewType === 'category' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Détails par Catégorie</h3>
                ...
                {categoryData.slice(0, 8).map((item, index) => (
                  <div key={item.category} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-gray-500">{item.percentage.toFixed(1)}% • {item.count} transactions</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {viewType === 'trends' && (
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Tendances d'Évolution</h3>
            {trendsData.length > 1 ? (
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={trendsData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Tooltip 
                    formatter={(value, name) => [
                      ...
                      name === 'incomeTrend' ? 'Tendance Revenus' : 'Tendance Dépenses'
                    ]}
                  />
                  <Line 
                    type="monotone" 
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Transaction moyenne</span>
              <span className="font-semibold">{formatCurrency(averageTransaction)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Plus grosse dépense</span>
              <span className="font-semibold">{formatCurrency(largestExpense)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Nombre de catégories</span>
              <span className="font-semibold">{categoryData.length}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Période analysée</span>
              <span className="font-semibold">
                {Math.ceil((selectedDateRange.end.getTime() - selectedDateRange.start.getTime()) / (1000 * 60 * 60 * 24))} jours
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            {categoryData.length > 0 && categoryData[0].percentage > 40 && (
              <div className="p-3 bg-warning/10 border border-warning/20 rounded-lg">
                <div className="text-sm text-warning font-medium">Catégorie dominante</div>
                <div className="text-xs text-warning mt-1">
                  {categoryData[0].category} représente {categoryData[0].percentage.toFixed(1)}% de vos dépenses.
                </div>
              </div>
            )}

            <div className="p-3 bg-financial-100 border border-financial-200 rounded-lg">
              <div className="text-sm text-financial-700 font-medium">Épargne recommandée</div>
